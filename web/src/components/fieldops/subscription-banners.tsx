...
import { isTrialEndingSoon, requestActivation } from "@/lib/subscription";
import { cn } from "cn";
  ...
  if (
    ...
    return (
      <Banner
        ...
        description={
          canManage
            ? "Contact us to activate your workspace. Online payment is not available."
            : "An owner or admin can request activation. Existing records stay visible."
        }
        action={
          <div className="flex flex-wrap gap-2">
            ...
            <Button asChild variant="outline" className="h-9 md:h-8">
              <Link href={`/app/${orgSlug}/settings`}>Account</Link>
            </Button>
          </div>
        }
      />
...
export function RequestActivationButton({
  ...
  return (
    <div className="flex flex-col items-start gap-1">
      <Button
        ...
        onClick={async () => {
          ...
          try {
            ...
            setMessage("Activation request sent. We will contact you shortly.");
            await refresh();
          } catch (error) {
            setMessage(
      ...
      >
        {pending ? "Sending…" : "Request Activation"}
      </Button>
...
export function TrialChip() {
  ...
  return (
    <span className="hidden rounded-full border border-border bg-muted px-2 py-0.5 text-[11px] font-medium text-muted-foreground lg:inline">
