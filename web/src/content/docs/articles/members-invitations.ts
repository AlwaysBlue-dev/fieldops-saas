...
export const membersInvitations: DocArticle = {
  ...
  sections: [
    ...
    },
    {
      ...
      paragraphs: [
        "Invitees accept from the invitation link. The invitation secret proves control of the invited email: new accounts are created already verified for that address, and an existing unverified account for the same email is marked verified on accept. A different signed-in email cannot accept the invite. Existing verified FieldKeel users join under their current account. New users set a password as part of acceptance, then land in that organization with the proposed role on their membership.",
